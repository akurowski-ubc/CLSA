...
df2 <- tibble::tibble(
  income_cat = c("100–149k", "50–99k", "20–49k", "<20k", "DN/NA/R"),
  estimate   = c(-0.097, 0.042, 0.137, 0.154, 0.154),
  conf.low   = c(-0.22, -0.07, 0.01, -0.03, -0.04),
  conf.high  = c(0.03, 0.16, 0.26, 0.34, 0.34),
  p_value    = c(0.136, 0.481, 0.036, 0.102, 0.113)
) %>%
  mutate(
    income_cat = factor(income_cat,
                        levels = c("<20k","20–49k","50–99k","100–149k","DN/NA/R")),
    sig = p_value < 0.05,
    lab = sprintf("%.3f  [%.2f, %.2f]", estimate, conf.low, conf.high)
  )


ggplot(df2, aes(x = estimate, y = income_cat, color = sig)) +
  geom_errorbarh(aes(xmin = conf.low, xmax = conf.high), height = 0.18, size = 1) +
  geom_point(size = 3.2) +
  geom_vline(xintercept = 0, linetype = "dashed", linewidth = 0.6, color = "grey40") +
  geom_text(aes(x = conf.high + x_pad, label = lab),
            hjust = 0, size = 4, color = "grey20") +
  scale_color_manual(values = c(`TRUE` = "#1f4e79", `FALSE` = "grey60")) +
  labs(
    title = "Association between Income Category and SO2",
    x = "SO2 level (β, 95% CI)",
    y = "Income Group"
  ) +
  ...
  theme(
    ...
    plot.title = element_text(face = "bold", hjust = 0.5, size = 18, margin = margin(b = 6)),
